# Чат-бот ресторана (учебный проект)

Консольный ассистент для гостей: ответы по меню и документам (RAG), бронирование столов в PostgreSQL, эскалация к менеджеру. LLM вызывается через **LangChain** с **function calling**; конфигурация — **Pydantic Settings** и **Docker Compose**.
