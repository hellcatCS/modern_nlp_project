...
-   Молодые люди чаще более активно настроены на взаимодействие через мессенджер, чем через звонок
...
Возможно добавление имплементация гибридного поиска (добавление результатов BM25) в зависимости от получаемого в процессе качества.

## Реранкер стадия (Опционально)

Для повышения качества выдачи retrieval этапа при большом индексе возможно добавление reranker стадии (отдельной более тяжеловесной модели)  
Для этого набор кандидатов, полученный из векторной базы, переранжируется с учетом скореа ранкер модели и дополнительной бизнес логики (например, мы можем хотеть рекомендовать свежии позиции блюд из меню чаще, в данном случае кадидаты с более новой датой должны получать больший скор)
...
-   Хранение в векторной БД (например, Qdrant/ElasticSearch)
...
-   Стабильна на большом контексте
-   Хорошо умеет в tool calling
